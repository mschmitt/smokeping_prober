...
func (s *smokePingers) prepare(hosts *[]string, interval *time.Duration, privileged *bool, sizeBytes *int, mark *uint) error {
	...
	for _, targetGroup := range sc.C.Targets {
		...
		for _, host = range targetGroup.Hosts {
			...
			pinger.SetMark(targetGroup.Mark)
...
func main() {
	var (
		...
		mark       = kingpin.Flag("ping.mark", "Ping firewall mark (for policy routing)").Short('m').Default("0").Uint()
	...
	err = smokePingers.prepare(hosts, interval, privileged, sizeBytes, mark)
	if err != nil {
